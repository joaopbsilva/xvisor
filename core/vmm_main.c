...
#define BOOTCMD_WIDTH		256
	...
	if (node) {
		...
		//while(line != "exit")vmm_gets(line, 256, '\n', NULL, 1);
#if defined(CONFIG_RTC)
...
static void system_init_work(struct vmm_work *work)
{
	...
	ret = vmm_wallclock_init();//direito
	if (ret) {
		goto fail;
	}
	...
	ret = arch_cpu_final_init();//Não faz nada
	if (ret) {
		goto fail;
	}
...
/*static vmm_irq_return_t aaa(int irq_no, void *dev)
{

	vmm_printf("aaaaaaaaaaaaaaaaa\n");
...
static void __init init_bootcpu(void)
{
	...
	ret = vmm_host_aspace_init();//Funciona direito
	if (ret) {
	...
	ret = vmm_heap_init();//funciona direito
	if (ret) {
		goto init_bootcpu_fail;
	}
	...
	ret = vmm_host_irq_init();//Interrupção gerada corretamente depois disto...
	vmm_printf("VBAR = %u \n",read_vbar());
	if (ret) {
		goto init_bootcpu_fail;
	}
...
/*Generate Interrupt!*/
//	vmm_host_irq_register(3, "aaa",&aaa, NULL);
//	vmm_host_generic_irq_exec(3);
/*********************/

	/* Initialize CPU early */
	vmm_printf("init: CPU early\n");//Apenas vai buscar algumas coisas à dts...
	...
	/*
	 * Não preciso fazer nada para já...
	 * */

	/*vmm_printf("init: board early\n");
	//ret = arch_board_early_init();//<-----Para Aqui!!
	if (ret) {
		goto init_bootcpu_fail;
	}*/
	...
	/*
	 * Para já vou deixar isto de parte visto que dá pra imprimir
	 * coisas com o early defterm, quando precisar altero este driver.
	 * */
	...
	vmm_printf("init: vmm_timer_start\n");
