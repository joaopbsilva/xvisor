...
static void timer_clockchip_event_handler(struct vmm_clockchip *cc)
{
	...
	tlcp->inprocess = TRUE;
//	vmm_printf("timer_clock_chip_event_handler_timer_name = %s\n",tlcp->tc.cs->name);
...
void vmm_timer_start(void)
{
	u64 tstamp;
	struct vmm_timer_local_ctrl *tlcp = &this_cpu(tlc);
	vmm_printf("clockchip name : %s\n", tlcp->cc->name);
	vmm_clockchip_set_mode(tlcp->cc, VMM_CLOCKCHIP_MODE_ONESHOT);
//	vmm_printf("timestampaaaaaaaaaaaaaa\n");
	tstamp = vmm_timer_timestamp();

	tlcp->next_event = tstamp + tlcp->cc->min_delta_ns;
//	vmm_printf("started = true\n");
	tlcp->started = TRUE;
//	vmm_printf("vmm_clockchip_program_event\n");
...
//	vmm_printf("clockchip bind best = %s\n", tlcp->cc->name);
	if (!tlcp->cc) {
