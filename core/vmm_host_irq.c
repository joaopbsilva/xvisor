...
{
	struct vmm_host_irq *irq;
	vmm_printf("irq_raise \n");
	if (NULL == (irq = vmm_host_irq_get(hirq)))
		return VMM_ENOTAVAIL;
	vmm_printf("irq_raise = %p || irq - name = %s\n", irq->chip->irq_raise, irq->chip->name);
	if (irq->chip && irq->chip->irq_raise) {
		vmm_printf("irq_raise = %p\n", irq->chip->irq_raise);
		irq->chip->irq_raise(irq, dest);
	}
	vmm_printf("irq_raise vmm_ok\n");
	return VMM_OK;
}
			  ...
			  void *dev)
{
	...
	if (NULL == (irq = vmm_host_irq_get(hirq)))
		return VMM_ENOTAVAIL;
	vmm_printf("host_irq_register\n");
	if (vmm_host_irq_is_per_cpu(irq)) {
		vmm_printf("host_irq_is_per_cpu\n");
		...
		vmm_printf("host_irq_is_per_cpu else\n");
					...
					void *data)
{
	...
	vmm_printf("host_irq_found - init_fn = %p\n", init_fn);
	err = init_fn(node);
...
int __cpuinit vmm_host_irq_init(void)
{
	...
	if (vmm_smp_is_bootcpu()) {
		...
		hirqctrl.matches = 
			vmm_devtree_nidtbl_create_matches("host_irq");//drivers/irqchip/irq-gic.c
		vmm_printf("vmm_devtree_nidtbl_create_matches host irq\n");
