		       ...
		       virtual_addr_t *addr, int regset)
{
	...
	rc = vmm_devtree_regsize(node, &sz, regset);//Vai buscar o size de memoria necessario
	if (rc) {
		return rc;
	}

	rc = vmm_devtree_regaddr(node, &pa, regset);//Vai buscar o endereço inicial da memoria
	if (rc) {
		return rc;
	}
