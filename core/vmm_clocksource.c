...
//	vmm_printf("timestamp read name couter = %s\n",tc->cs->name);
	cycles_now = tc->cs->read(tc->cs);
	cycles_delta = (cycles_now - tc->cycles_last) & tc->cs->mask;
...
int vmm_timecounter_start(struct vmm_timecounter *tc)
{
	...
	vmm_printf("clocksource enable = %p\n",tc->cs->enable);//no caso do gpt nao tem enable
	if (tc->cs->enable) {
		tc->cs->enable(tc->cs);
	}
					...
					void *data)
{
	...
	vmm_printf("clockSource_nidtbl_found init_fn = %p\n", init_fn);
