...
//	vmm_printf("host_memunmap va = %lu , sz = %lu\n", va, sz);
	sz = VMM_ROUNDUP2_PAGE_SIZE(sz);
	va &= ~VMM_PAGE_MASK;
//	vmm_printf("host_memunmap 1\n");
	if ((rc = arch_cpu_aspace_va2pa(va, &pa))) {
		return rc;
	}
//	vmm_printf("host_memunmap 2\n");
...
//	vmm_printf("vmm_host_free_pages, page_va = %u , page_count = %u\n", page_va, page_count);
	if ((rc = arch_cpu_aspace_va2pa(page_va, &pa))) {
		return rc;
	}
//	vmm_printf("vmm_host_free_pages 1\n");
	if ((rc = host_memunmap(page_va, page_count * VMM_PAGE_SIZE))) {
		return rc;
	}
//	vmm_printf("vmm_host_free_pages 2\n");
