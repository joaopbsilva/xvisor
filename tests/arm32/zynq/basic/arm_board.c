...
{
	return 0x40000000;//Tenho de meter menos ram que o total aqui?
}
...
{
	arm_strcpy(cmdline, "console=tty0 console=ttyPS0,115200 "
			"root=/dev/ram rw initrd=0x20800000,8M  earlyprintk");
	//arm_strcpy(cmdline, "console=ttymxc0,115200 mem=128M rw root=/dev/mmcblk0p2");
...
u32 arm_board_flash_addr(void)//Apenas é preciso caso use o comando autoexec
{
	return (u32)(IMX_NOR);
}
...
{
	return arm_priv_timer_enable();
}

void arm_board_timer_disable(void)
{
	return arm_priv_timer_disable();
}

u64 arm_board_timer_irqcount(void)
{
	return arm_priv_timer_irqcount();
}

u64 arm_board_timer_irqdelay(void)
{
	return arm_priv_timer_irqdelay();
}

u64 arm_board_timer_timestamp(void)
{
	return arm_priv_timer_timestamp();
}

void arm_board_timer_change_period(u32 usecs)
{
	return arm_priv_timer_change_period(usecs);
}

int arm_board_timer_init(u32 usecs)
{
	arm_board_pic_unmask(IRQ_PRIVATE_TIMER);

	return arm_priv_timer_init(usecs, ZYNQ_PRIVATE_TIMER, IRQ_PRIVATE_TIMER, 0);
}

#define	ZYNQ_UART_BASE		ZYNQ_UART1
#define	ZYNQ_UART_INCLK		50000000
#define	ZYNQ_UART_BAUD		115200

int arm_board_serial_init(void)
{
	zynq_init(ZYNQ_UART_BASE, ZYNQ_UART_BAUD, ZYNQ_UART_INCLK);
...
{
	if (ch == '\n') {
		zynq_putc(ZYNQ_UART_BASE, '\r');
	}

	zynq_putc(ZYNQ_UART_BASE, ch);
}

char arm_board_serial_getc(void)
{
	char ch = zynq_getc(ZYNQ_UART_BASE);
	if (ch == '\r') {
		ch = '\n';
	}
