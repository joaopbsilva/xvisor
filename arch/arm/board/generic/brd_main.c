				       ...
				       void *data)
{
	...
	vmm_printf("generic board early\nboard = Name = %s || early_init = %p || final_init = %p || print_info = %p\n", brd->name, brd->early_init, brd->final_init, brd->print_info);
	err = brd->early_init(node);
	vmm_printf("generic board early 1\n");
	if (err) {
...
int __init arch_board_early_init(void)
{
	 ...
	 * ....
	 */
	vmm_printf("init: board early 1\n");

	/* Determine generic board matches from nodeid table */
	generic_board_matches = 
		vmm_devtree_nidtbl_create_matches("generic_board");
	if(generic_board_matches == NULL)
		vmm_printf("generic_board null\n");
	vmm_printf("init: board early 2\n");
	...
	vmm_printf("init: board early || %s || %s || %s || %p\n", generic_board_matches->compatible, generic_board_matches->name, generic_board_matches->type, generic_board_matches->data);

	/* Initialize clocking framework */
	of_clk_init(NULL);
	vmm_printf("init: board early 4\n");
	return VMM_OK;
}
...
int __init arch_board_final_init(void)
{
	...
	vmm_printf("arch board final init\n");
	...
	vmm_printf("arch board final init 11111111111111\n");
	/* Create VTEMU instace if available */
#if defined(CONFIG_VTEMU)
	vmm_printf("config vt emu\n", brd->final_init);
	info = fb_find("fb0");
	if (info) {
		generic_vt = vtemu_create(info->name, info, NULL);
	}
#endif
	vmm_printf("arch board final init1\n");
