...
{
	int rc;
	vmm_printf("imx_defterm_init base address = %lu\n", imx_defterm_base);
	...
	rc = vmm_devtree_clock_frequency(node,
				&imx_defterm_inclk);
	vmm_printf("imx_defterm_init clock frequency = %u\n", imx_defterm_inclk);
	if (rc) {
		return rc;
	}
	...
	vmm_printf("imx_defterm_init baudrate = %u\n", imx_defterm_baud);
...
int __init arch_defterm_init(void)
{
	...
	vmm_printf("arch_defterm_init\n");
	if (!node) {
		return VMM_ENODEV;
	}
	vmm_printf("arch_defterm_init 1\n");
	if (!vmm_devtree_is_available(node)) {
		return VMM_ENODEV;
	}
	vmm_printf("arch_defterm_init 2\n");
