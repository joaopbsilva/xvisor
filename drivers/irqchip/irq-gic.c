...
static void __init gic_dist_init(struct gic_chip_data *gic)
{
	...
	virtual_addr_t base = gic->dist_base;
	vmm_printf("Gic distributor address = %lu\n", base);
				 ...
				 virtual_addr_t dist_base)
{
	...
	vmm_printf("Gic dist init\n");
	gic_dist_init(gic);
	vmm_printf("Gic cpu init\n");
	gic_cpu_init(gic);
				   ...
				   bool eoimode)
{
	...
	if (rc) {
		vmm_printf("gic_devtree_init gic_cpu2\n");
	...
	vmm_printf("cpu2_base = %lu\n", cpu2_base);
	vmm_printf("cpu_base = %lu\n", cpu_base);
	vmm_printf("dist_base = %lu\n", dist_base);
	if (vmm_devtree_read_u32(node, "irq_start", &irq_start)) {
		vmm_printf("irq_start\n");
		irq_start = 0;
	}
	vmm_printf("irq_start = %u\n", irq_start);
	...
	if (parent) {
		...
		vmm_printf("parent existing\n");
		gic_cascade_irq(gic_cnt, irq);
	} else {
		vmm_printf("vmm_host_irq_set_active_callback\n");
...
static int __cpuinit gic_init(struct vmm_devtree_node *node)
{
	...
	vmm_printf("gic init - node name - %s\n", node->name);
	return rc;
}
