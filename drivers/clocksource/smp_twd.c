...
static int __cpuinit twd_clockchip_init(struct vmm_devtree_node *node)
{
	...
	vmm_printf("twd_clockchip init1\n");
	if (!twd_ppi_irq) {
	...
	vmm_printf("twd_clockchip init2\n");
	twd_freq_hz = 333333000;
	//twd_freq_hz = 400000000;
	if (!twd_freq_hz) {
		vmm_printf("not twd_frequency \n");
	...
	vmm_printf("twd_clockchip init3\n");
	...
	cc->clkchip.priv = cc;
	vmm_printf("twd_clockchip init4\n");
	...
	vmm_printf("twd_clockchip init5\n");
	rc = vmm_clockchip_register(&cc->clkchip);
	if (rc) {
		goto fail_unreg_irq;
	}
	vmm_printf("twd_clockchip init6\n");
	return VMM_OK;
