...
void zynq_uart_lowlevel_init(struct zynq_uart_priv *port)
{
       struct uart_zynq *regs = port->regs;
       /* RX/TX enabled & reset */
       vmm_writel(ZYNQ_UART_CR_TX_EN | ZYNQ_UART_CR_RX_EN | ZYNQ_UART_CR_TXRST | \
                                       ZYNQ_UART_CR_RXRST, &regs->control);
       vmm_writel(ZYNQ_UART_MR_PARITY_NONE, &regs->mode); /* 8 bit, no parity */
                                  ...
                                  const struct vmm_devtree_nodeid *devid)
{
       ...
       rc = vmm_devtree_request_regmap(dev->of_node, (virtual_addr_t*)&port->regs, 0,
                                       "ZCU102 UART");
       if (rc) {
               goto free_port;
       }
...
free_reg:
       vmm_devtree_regunmap_release(dev->of_node,(virtual_addr_t)port->regs, 0);
free_port:
       vmm_free(port);
free_nothing:
       return rc;
